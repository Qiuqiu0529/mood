...
def train_and_evaluate_model(model_name, model, params, X_train, y_train, X_val, y_val, feature_type):
    ...
    print(f"Training Accuracy: {train_acc:.4f}, Training Log Loss: {train_loss:.4f}")
    print(f"Validation Accuracy for {model_name} with {feature_type} features: {val_acc:.4f}, Validation Log Loss: {val_loss:.4f}")
    ...
    train_util.draw_confusion_matrix_hitmap(y_val, y_val_pred, title, cm_path)


    # Log metrics
    train_util.log_metrics(model_name=model_name, feature_type=feature_type, train_acc=train_acc,train_loss=train_loss,
                           val_acc=val_acc, val_loss=val_loss,class_report=class_report, train_time=train_time,
                           params=grid.best_params_,
                           cm_path=cm_path)

    return val_acc, train_acc, train_loss, val_loss, best_model, grid.best_params_
...
for model_name, model_info in models.items():
    val_acc, train_acc, train_loss, val_loss, _, _ = train_and_evaluate_model(
        model_name, model_info['model'], model_info['params'],
        X_train_tf, y_train, X_val_tf, y_val, 'TF-IDF'
    )
    acc_tf[model_name] = val_acc
    model_results[model_name] = (train_acc, train_loss, val_loss, val_acc)
...
if acc_tf[best_model_name_tf] >= acc_bag[best_model_name_bag]:
    ...
    train_acc, train_loss, val_loss, val_acc = model_results[best_model_name_tf]
else:
    ...
    train_acc, train_loss, val_loss, val_acc = model_results[best_model_name_bag]
...
print(f"Test Log Loss for {best_model_name} with {feature_type} features: {test_log_loss:.4f}")
...
train_util.log_metrics(
    ...
    train_time=train_util.get_train_time_from_log(best_model_name, feature_type),
    params=grid.best_params_,
